...
class TCA:
    ...
    def tca_plus(self, Xs, Xt):
        ...
        if sort_degree(mean1, mean2) == 4 and sort_degree(std1, std2) == 4:
        ...
        elif (sort_degree(min1, min2) == 7 or sort_degree(min1, min2) == 1) and (
                sort_degree(max1, max2) == 7 or sort_degree(max1, max2) == 1) and (
                sort_degree(ns, nt) == 7 or sort_degree(ns, nt) == 1):
            X = np.vstack((Xs, Xt))
            X = (X - X.min(axis=0)) / (X.max(axis=0) - X.min(axis=0))
            X = X.T
            print("N2")
            return X
        elif (sort_degree(std1, std2) == 7 and sort_degree(ns, nt) < 4) or (
                sort_degree(std1, std2) == 1 and sort_degree(ns, nt) > 4):
            Xs = (Xs - mean1) / std1
            Xt = (Xt - mean1) / std1
            X = np.vstack((Xs, Xt))
            X = X.T
            print("N3")
            return X
        elif (sort_degree(std1, std2) == 7 and sort_degree(ns, nt) == 7) or (
                sort_degree(std1, std2) == 7 and sort_degree(ns, nt) == 1):
            Xs = (Xs - mean2) / std2
            Xt = (Xt - mean2) / std2
            X = np.vstack((Xs, Xt))
            X = X.T
