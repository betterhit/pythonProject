...
def sort_degree(x, y):
    ...
    if x * 1.6 < y:
        ...
        return 1;
class TCA:
    def __init__(self, kernel_type='primal', dim=15, lamb=1, gamma=1):
    ...
    def fit_predict(self, Xs, Ys, Xt, Yt, X):
        ...
        auc = roc_auc_score(Yt, y_pred, average='weighted')
        return f1, y_pred
...
if __name__ == '__main__':
    file_path = 'sort_final_f1.csv'  # CSV文件路径
    ...
    with open('new_data.txt', 'r') as file:
        ...
        for line in lines:
            ...
            file_name = elements[0]
            file_name1 = elements[1]
            columns = list(map(int, elements[2].split(',')))
            file_columns1.append([file_name,file_name1,columns])
    # print(file_columns1)
    ...
    for file1, file2 in zip(list1, list2):
        for item1 in  file_columns1:
            file_name = item1[0]
            file_name1 = item1[1]
            columns_to_select = item1[2]


            # file_name = item[0]
            # columns_to_select = item[1]

            # if file2 == file_name and len(item[1])>8:
            if file2 == file_name and file1 == file_name1:

                columns_num =  [item[1] for item in file_columns if item[0] == file_name]
                columns_num = [item[1] for item in file_columns if item[0] == file_name]
                if len(columns_num[0])>9:
                    ...
                    # 选择特定列
                    Xs = df2.iloc[:, columns_num[0]].values
                    Ys = df2.iloc[:, 21].values
                    Ys[Ys != 0] = 1

                    Xt = df1.iloc[:, columns_num[0]].values
                    ...
                    print(acc1)
                    list3.append(acc1)
            # elif file2 == file_name and len(item[1]) <= 8:
                else:
                    file_path1 = f'../../bugdata3/{file1}'
                    file_path2 = f'../../bugdata3/{file2}'
                    df1 = pd.read_csv(file_path1)
